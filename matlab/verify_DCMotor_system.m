...
[t_ode, x_ode] = ode45(sys, [t_input(1):0.001:t_input(end)], x0);
% 補插值：確保 RK4 輸出長度與 C++ 一樣（方便比較）
state0_rk4 = interp1(t_rk4, x_rk4(:,1), time_cpp);
state1_rk4 = interp1(t_rk4, x_rk4(:,2), time_cpp);

%% === Plot 1：位置比較 ===
figure;
plot(time_cpp, x_ode(:,1), 'LineWidth', 1.8, 'DisplayName', 'C++ Implementation'); hold on;
plot(time_cpp, x_rk4(:,1), '--', 'LineWidth', 1.8, 'DisplayName', 'MATLAB RK4');
...
plot(time_cpp, x_rk4(:,2), '--', 'LineWidth', 1.8, 'DisplayName', 'MATLAB RK4');
xlabel('Time (s)'); ylabel('Velocity');
title('Velocity Comparison: C++ vs MATLAB RK4');
legend('Location','best'); grid on; box on;

%% === Plot 3：位置誤差 ===
position_error = x_rk4(:,1) - x_ode(:,1);
figure;
...
%% === Plot 4：速度誤差 ===
velocity_error = x_rk4(:,2) - x_ode(:,2);
figure;
